...
class DynamodbTraceRecordManager:
    ...
    def get_trace_contexts(self, trace_id: str) -> tuple[TraceRecord.ContextResolution]:
        ...
        contexts_resolutions = deserialize(dynamodb_item['Item']['contexts_resolutions'])
        return tuple([
    ...
    def store_in_progress(self, trace_id: str, command: dict, contexts_resolutions: tuple[str]):
        ...
        item = {
            ...
            'Item': {
                ...
                'version': serialize(1),
                'timestamp': serialize(datetime.datetime.timestamp(datetime.datetime.now())),
    ...
    def store_context_resolve_success(self, trace_id: str, context: str):
        item = {
            ...
            'UpdateExpression': f'SET contexts_resolutions.{context}.resolution = :resolution',
            'ExpressionAttributeValues': {
                ':resolution': serialize(TraceRecord.Resolution.success.name)
            }
        }
