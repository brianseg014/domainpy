...
from domainpy.domain import DomainEvent, DomainError
...
class EventSourcedEnvironment:
    ...
    def __init__(
        ...
        *,
        ...
        self.setup_domain_publisher_bus(PublisherBusAdapter(self.domain_publisher_bus), self.event_mapper, self.setupargs)
        self.setup_integration_publisher_bus(PublisherBusAdapter(self.integration_publisher_bus), self.integration_mapper, self.setupargs)
        self.setup_projection_bus(ProjectionBusAdapter(self.projection_bus), self.setupargs)
        self.setup_resolver_bus(ApplicationBusAdapter(self.resolver_bus), self.integration_publisher_bus, self.setupargs)
        self.setup_handler_bus(ApplicationBusAdapter(self.handler_bus), self.registry, self.setupargs)
